...
- **Risk:** Multiple `dut.led.value` reads per frame changes the simulation timing model. Each read requires a cocotb `Timer` await, so N sub-steps per frame multiplies the per-step simulation cost by N. This lowers the child's sim throughput (the host's ~62 fps render is unaffected since U34). Mitigate by making sub-step count configurable (default 1 = current behavior, opt-in to N > 1 for PWM designs). Benchmark before/after.
- **Touches (re-pointed after U34):** `sim/sim_testbench.py` (headless child — sample `dut.led` duty cycle into the `state` stream), `src/fpga_sim/ui/simulation_screen.py` (apply streamed brightness), `src/fpga_sim/ui/components.py` (`LED.draw`).
- **Effort:** M.
...
- **Why:** A machine can host four working simulators — GHDL mcode / LLVM / LLVM-JIT (all proven against the app since PRs #262/#264) plus NVC — with real trade-offs: NVC is fastest everywhere (~4–6x mcode), GHDL-LLVM is ~2–3x mcode but pays a compile+link per launch and per Reload VHDL, LLVM-JIT is a free ~1.2x with mcode's instant startup (honest numbers 2026-07-18, `memory/project_sim_performance.md`). Today the only selector is `shutil.which("ghdl")` — invisible, not persistable, PATH surgery to change, and the session log can't record which GHDL ran.
- **What:** (1) **Discovery** — probe `ghdl`/`nvc` on PATH plus conventional locations (`/usr/local/ghdl-*/bin/ghdl`, distro `ghdl-llvm`-style names); identify each GHDL by its `--version` backend line; de-dup by realpath; label "GHDL (mcode)" / "GHDL (LLVM)" / "GHDL (LLVM JIT)" / "NVC". (2) **Registration** for undiscoverable installs — `extra_simulators` list in the session file, `FPGA_SIM_EXTRA_SIMS` env for one-offs, and `fpga-sim --add-sim <path>` sugar (no new text-input widget; a picker-based "Browse…" is a parked phase-2 idea). (3) **Selection** — the preview screen's existing `[SIM:…]` toggle cycles every entry; the choice (engine + path) persists in the session with a safe PATH-default fallback; `--sim` gains `ghdl-mcode|ghdl-llvm|ghdl-jit` slugs or a path; new `--list-sims`. (4) **Compiled-backend validation parity** — a stage-3 runtime-elaboration probe (run the `-e`-produced executable `--stop-time=0fs` when one exists in the workdir) maps "bound check failure" into the existing hint pipeline, restoring load-time rejection on every backend and fixing the 3 stage-3 hint tests; the 2 direct-invocation tests gain an `-e` step (together: the 5 mcode-assumption slow tests). (5) **CI** — a Linux GHDL-LLVM job from the official `ghdl-llvm-6.0.0-ubuntu24.04` release asset (same SHA-pin pattern as the existing mcode step), non-required until proven. (6) **Docs** — install.md "Choosing a simulator" (relative-performance table, which-to-install guidance, backend build recipe) + a user_guide echo.
- **Touches:** `sim_bridge.py` (discovery/identity; builders use the chosen binary; stage-3 probe), `ui/board_display.py` (`[SIM:…]` labels/cycle), `controller.py` + `session_config.py` (persist engine + path), `__main__.py` (`--sim` slugs, `--list-sims`, `--add-sim`), `sim_session_log.py` (backend/path fields), tests (+ the 5 backend-agnostic fixes), `.github/workflows/`, `docs/install.md`, `docs/user_guide.md`.
- **Effort:** M.
- **Dependencies:** none (#262/#263/#264 cleared compiled-backend operation). The `Simulator` widening must follow the full typed round-trip (preview toggle → controller → session → builders → benchmark → session log).
- **Done when:** every installed simulator appears truthfully labeled in `[SIM:…]` and `--list-sims`; a manually registered binary at an arbitrary path is selectable and honored end-to-end (launch, benchmark, session log); a bad design is rejected at load time on all backends; the slow suite passes under mcode, LLVM, and LLVM-JIT locally; the CI LLVM job is green. Execution plan: [u35_simulator_picker_plan.md](u35_simulator_picker_plan.md).
...
- **Touches:** `src/fpga_sim/sim_bridge.py`; tests extend `tests/test_sim_child.py`.
...
- **Touches:** new `src/fpga_sim/sandbox.py`; `src/fpga_sim/sim_bridge.py` (`start_simulation` — wrap the headless child `Popen` (U34); optionally the analyze/elaborate runs); `session_config.py` / Settings (**U5** ✅) for the toggle; a docs note that HDL is executable code; new `tests/test_sandbox.py`; one Linux-only CI job.
...
- `src/fpga_sim/__main__.py` — U2 ✅, U5 ✅ (window-size restore), U21 ✅ (`res.match` → analyze/launch), U16, D6a ✅, D6b ✅ (now a thin driver), D9 ✅, U35 (`--sim` slugs + `--list-sims` + `--add-sim`)
...
- `sim/sim_testbench.py` — U34 ✅ **replaced it with the headless bridge** (no pygame; streams state over `sim_link`; the pre-U34 pygame-testbench notes U5/U7/U21 ✅ are history — those UI surfaces now live in `ui/simulation_screen.py`), U9 (child-side duty sampling), U22
- `src/fpga_sim/ui/simulation_screen.py` — U34 ✅ (new: in-launcher `SimulationScreen` + `RunStats`), U23 (dirty-flag draw loop), U9 (apply streamed brightness), U14 (`P` pause key)
