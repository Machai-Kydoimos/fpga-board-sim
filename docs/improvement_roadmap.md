...
| **P4** | Python 3.14 in the CI matrix | **cocotb** ships `cp314` wheels (the pygame half is already satisfied) | S | Re-checked 2026-08-24, after the move to pygame-ce: pygame-ce 2.5.8 ships `cp310`–`cp315` wheels, so **cocotb 2.0.1 is now the sole blocker** — it still tops out at `cp313`, and adding 3.14 breaks `uv sync` on a cocotb sdist build. (The original blocker was upstream pygame 2.6.1, whose last release — 2024-09-29 — predates Python 3.14 entirely.) Re-check PyPI wheel *tags*, not release dates, before bumping the matrix upper bound. |
