...
6502 assembly programs for the single-file embedded-core systems (`hdl/mx65_*.vhd`), assembled with
...
| File | Role |
|------|------|
| `mx65_walking_counter_7seg.s` | 6502 source: the walking counter (bouncing one-hot LED + BCD odometer; `btn(0)` reverse, `btn(1)` lamp test, switch speed) |
| `mx65.cfg` | ld65 config: a 2 KB ROM at `$F800-$FFFF` with CPU vectors at `$FFFA` |
| `mx65_walking_counter_7seg.bin` | assembled 2 KB ROM image — **the source of truth**, embedded into the VHDL |
...
Add `-l mx65_walking_counter_7seg.lst` to the `ca65` line for a source+address+bytes listing, or run
`da65 mx65_walking_counter_7seg.bin` to disassemble the image as a cross-check.
...
`hdl/mx65_walking_counter_7seg.vhd`:

```bash
uv run python scripts/embedded_core/rom_to_vhdl.py firmware/mx65_walking_counter_7seg.bin
```
