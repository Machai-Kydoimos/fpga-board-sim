"""Golden test for the U21 A2 constraint-dialect parsers + classifier.

Parses the complete, real Digilent Basys3 master XDC (commit 00a3404, the
same commit `boards/digilent-xdc/basys_3.json`'s ``source`` block cites) and
checks that the new dialect-agnostic pipeline (``xdc.parse`` +
``classify.classify``) reproduces that board's existing hand-generated
``port_conventions.digilent`` block on clk/leds/switches/buttons.

The 159-line fixture below is the file verbatim (not a trimmed excerpt, for
once): the whole point of a golden test is fidelity to the real source,
including all the Pmod/VGA/USB/QSPI sections a hand-picked excerpt would have
...
# Digilent Basys-3-Master.xdc, commit 00a3404901f35aa9567b01ecb3f2c233b6efe9f4
# (see boards/digilent-xdc/basys_3.json's source.sync_commit), fetched
# verbatim from https://raw.githubusercontent.com/Digilent/digilent-xdc/
# 00a3404901f35aa9567b01ecb3f2c233b6efe9f4/Basys-3-Master.xdc
_BASYS3_MASTER_XDC = """\
...
def test_reproduces_stored_clk_leds_switches_buttons() -> None:
...
def test_clock_frequency_matches_boards_default_clock_hz() -> None:
    table = parse(_BASYS3_MASTER_XDC)
    assert table.clocks[0].frequency_hz == 100e6
...
def test_decimal_point_and_pmod_headers_are_excluded() -> None:
    # "dp" is a lone decimal-point pin (no seg/hex substring -> not seg-interest);
    # JA/JB/JC/JXADC/vga*/Rs*/PS2*/Qspi* must not leak into any bucket. A leak
    # would show up as e.g. an inflated led/switch width, which the exact-match
    # assertions above would already catch -- this makes that guarantee explicit.
    table = parse(_BASYS3_MASTER_XDC)
    serialized = json.dumps(classify(table))
    for leaked in ("JA", "JB", "JC", "JXADC", "vga", "Rs", "PS2", "Qspi", '"dp"'):
