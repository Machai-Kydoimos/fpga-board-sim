...
- **Inspect mode — a name for every place on screen** (roadmap U55). **F3** labels
  the window: each region gets a short, typable address (`sim.panel.speed.slider`,
  `sim.board.led[5]`, `dlg.settings.duty-bars`) and a corner readout names whatever
  is under the cursor alongside the build, board, design, simulator, theme and window
  size. **F4** copies that line to the clipboard, so a question about the app can say
  *where* as precisely as a question about a document — and
  [`docs/ui_map.md`](docs/ui_map.md), generated by rendering the real screens rather
  than by scanning for what they would do, turns the address back into a file and a
  line. Works on all four launcher screens, in every dialog, on the analysis
  spinner and on the "it looks frozen" advisory, and every list row and help
  section has its own address; the addresses are
  ...
  **Shift+F4** copies a fuller record as JSON, for when something is actually
  ...
  `--benchmark` still reports 62.1 fps, and 78 `--screenshots` stills contained zero
  pixels of the overlay. Board LEDs, switches, buttons and digits are deliberately
  never badged — 57 names are less readable than none, and they already show their
  own labels — and a badge that would land on another is dropped rather than stacked.
...
[Unreleased]: https://github.com/Machai-Kydoimos/fpga-board-sim/compare/v0.22.0...HEAD
[0.22.0]: https://github.com/Machai-Kydoimos/fpga-board-sim/compare/v0.21.0...v0.22.0
...
[0.17.0]: https://github.com/Machai-Kydoimos/fpga-board-sim/compare/v0.16.0...v0.17.0
[0.16.0]: https://github.com/Machai-Kydoimos/fpga-board-sim/compare/v0.15.0...v0.16.0
[0.15.0]: https://github.com/Machai-Kydoimos/fpga-board-sim/compare/v0.14.0...v0.15.0
