...
| `src/fpga_sim/wrapper.py` | Generates `sim_wrapper` (generic / native / duty-spliced) + `analyze_vhdl` + `wrapper_is_stale`. **`analyze_vhdl` step 0 analyzes the picked design's folder-siblings first (U51)** — to a fixpoint, order discovered by *retrying* rather than by parsing `use` clauses, and a sibling that never compiles is dropped rather than fatal (the course testbenches do not build as shipped). Only the picked file is simulated. Noticing an *edited* sibling belongs to [Reload VHDL], not to `wrapper_is_stale`, which compares the rendered wrapper artifact only |
