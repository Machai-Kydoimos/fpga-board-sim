...
| `src/fpga_sim/sim_bridge.py` | **Re-export shim (D17)** over the nine modules below — its docstring is the section map. ~35 files import through it and still work; new code should name the owning module. **A monkeypatched module global is the one thing it cannot forward** — patch `sim_discovery` / `waveform` / `sim_runner` directly |
| `src/fpga_sim/paths.py` | `REPO_ROOT` + `HDL_DIR` / `BOARDS_DIR` / `SIM_DIR` / `VENV_DIR`, derived once. **Never re-derive the root** — `tests/test_paths.py` walks the AST of every module to enforce it (a `parent.parent.parent` chain breaks silently when a module moves) |
| `src/fpga_sim/sim_config.py` | `Simulator` / `WaveFormat` / `DutyMode` types, `WaveConfig`, and the `FPGA_SIM_DUTY*` policy |
| `src/fpga_sim/sim_backends.py` | `_SimBackend` ABC + `_GHDLBackend` / `_NVCBackend`: how each engine spells analyze / elaborate / run |
| `src/fpga_sim/sim_discovery.py` | Which simulator installs exist on this machine, probed and labeled by code generator (U35) |
| `src/fpga_sim/vhdl_interface.py` | Encoding check + toplevel port/generic parsing (`_parse_toplevel_interface` → `_IfaceDecl`); pure text work, no policy |
| `src/fpga_sim/conventions.py` | Board-native matcher (U21): a design's own port names vs. the board's `port_conventions`, full match or nearest near-miss |
| `src/fpga_sim/vhdl_contract.py` | `check_vhdl_contract` + `add_error_hints` — the decision and, more importantly, the message |
| `src/fpga_sim/wrapper.py` | Generates `sim_wrapper` (generic / native / duty-spliced) + `analyze_vhdl` + `wrapper_is_stale`. **`analyze_vhdl` analyzes the picked design's folder-siblings on demand (U51)** — *lazily*: only after the design fails to analyze **or** to elaborate (a `component` with default binding fails at the second, not the first), because an eager sweep is free on mcode/NVC and **5x** on GHDL's AOT LLVM backend, where analyze == compile — to a fixpoint, order discovered by *retrying* rather than by parsing `use` clauses, and a sibling that never compiles is dropped rather than fatal (the course testbenches do not build as shipped). Only the picked file is simulated. Noticing an *edited* sibling belongs to [Reload VHDL], not to `wrapper_is_stale`, which compares the rendered wrapper artifact only |
| `src/fpga_sim/waveform.py` | VCD/FST capture, the `.gtkw` sidecar, viewer launch |
| `src/fpga_sim/sim_runner.py` | The child's environment (PATH / VPI / libpython), `start_simulation()` → `SimChild`, `finish_waveform()` |
...
| `src/fpga_sim/ui/` | pygame UI package (board_selector, board_display, simulation_screen, sim_panel, components, etc.). **Input model (U44):** a `Button` is held by a *set* of hold sources (`mouse:1` / `key:s30` / `latch`) and is down while that set is non-empty, so multi-hold, arbitrary release order, latching and keyboard holds all fall out of one idea; `FPGABoard` keys its `_mouse_holds` / `_key_holds` registries by widget **index**, never by rect, so a mid-gesture resize survives |
| `src/fpga_sim/ui/simulation_screen.py` | `SimulationScreen`: renders the board in the launcher's window while the headless child streams signal state over `sim_link`; returns a `SimExit` (U34). Hosts the **"it looks frozen" advisory** (U48): `_sample_stall` feeds `stall.StallWatch` the board's **`output_signature()`** — not `visual_signature()`, whose switch/button state a user changes — and the **offer indicator** (a drawn circle-i beside [Pause]; the font has neither U+24D8 nor U+2139) is part of the U23 frame signature, so it can appear on a frame that would otherwise be skipped. The advisory never interrupts: it is a control the user may click, and inputs are **recorded, never acted on** — they pick which explanation leads, and never reset the timer or withdraw the offer |
| `src/fpga_sim/ui/keymap.py` | Keyboard→button bindings (U44): hex `0`-`9`/`A`/`B`/`C` (+ numpad), key *d* pressing button **index** d. **Two tiers by necessity** — digits bind by *scancode* (the digit row is shifted on AZERTY/Czech) and letters by *key code* (pygame has no scancode→keycap lookup, so a scancode-bound letter can't be badged accurately). A target is resolved once at KEYDOWN and *looked up* at KEYUP, never recomputed, or a chord released modifier-first leaks its hold. Hold tokens are tier-prefixed because scancodes and key codes share one integer space. **All input state comes from the event stream** — `tests/test_no_input_polling.py` enforces that as an AST shape rule |
| `src/fpga_sim/ui/inspect.py` | Inspect mode (U55): **F3** paints a typable address on every region (`sim.panel.speed.slider`), **F4** copies it plus a context stamp; **shift-F4** copies a JSON record adding the widget's own value — for an LED **both** the measured duty and the displayed level, because with `led_pwm` off the renderer gets the raw bit (42% duty displays 100%) and with it on the level is a POV EMA, so reporting one as the other is a lie in *both* modes; same pair, same reason as #388's manifest — the design's mode + generics, and the run state — **no host/toolchain block, because `--doctor` already reports it and the record's `more` field says so**. Two formats because a review pastes a dozen findings inline while a defect report wants everything. Two orthogonal halves — an address that encoded state would be untypable, state left out would make it unreproducible. **Identity is never geometry** (`_layout` reassigns every rect on resize), and the registry is a module global like `THEME`/`_DEBUG_VIEW` so `draw_button` — one primitive for **17 call sites** — registers without being handed one. Three kinds: `zone` (frame + label), `item` (chrome, badged), `widget` (board parts, **never badged** — 57 names are less readable than none). Every entry point is a no-op on one bool read while off; colliding labels are dropped, never stacked. Colors are fixed, not THEME roles: a theme could restyle diagnostic chrome into invisibility. **Type scales off `_ui_scale` like every other widget** — fixed sizes shrink *relative to* a growing window, which reads as unreadable at full screen; **Shift+F3** cycles the overlay's own size (5 steps, wraps) and *persists* it — the size describes the display, unlike on/off, which does not. `FPGA_SIM_INSPECT_SCALE` (0.5–4.0) sets a starting value; deliberately **not** env-wins, unlike the waveform settings, because there the variable exists so CI can force a choice while here it seeds a reader who is about to adjust by eye. Read `mod` with `getattr` — a synthesized KEYDOWN has none, same trap as `unicode` |
| `scripts/gen_ui_map.py` + `docs/ui_map.md` | Generated `address → file:line`, by **rendering the real screens** (the `--doctor` rule) — a static scan cannot resolve an f-string path or a shared widget's host-chosen scope. Origins come from `inspect.set_trace_origins`, a generator-only flag that walks *out* of the registration plumbing, so a button's origin is its caller rather than the shared painter. `tests/test_ui_map.py` fails when the file and the product disagree — including after an import reorder shifts a line |
| `src/fpga_sim/version.py` | `app_version()` → `0.22.0+g9c6590e`. The project's **first** version stamp in any artifact (session logs, `manifest.json` and `session.json` all lack one); `git describe` is shelled out once behind an `lru_cache`, never per frame, and every failure degrades to the bare version |
| `src/fpga_sim/ui/clipboard.py` | `copy_to_clipboard` / `read_clipboard`, extracted from `error_dialog`. **Never raises** — a clipboard can be genuinely absent. The deprecated `scrap.init()` was dropped (`put_text`/`get_text` self-initialize), which is why this is a shared seam rather than a shared copy |
...
| `src/fpga_sim/ui/screenshots.py` | PNG capture for `--benchmark --screenshots DIR` (#129): the *visual* half of the board smoke-test. Saves the **product renderer's own** frames (the benchmark already drives the real `SimulationScreen` headless), so stills show true duty/RGB/scan brightness — unlike `sim/capture_frames.py`, whose binary `set_led` path is what Docs & Assets round 2 PR 3 exists to fix. The gate is the point: `visual_signature(quantize=COARSE_LEVELS)` (16 steps, not U23's 1000) + a 0.25 s floor between change shots, or a POV fade emits one PNG per frame; plus a 1 s liveness shot and a 240-file cap. `due()` is pure so the caller can fold it into U23's dirty test and capture **before** `flip`. Also writes the **`manifest.json` sidecar** (#388): per shot the duty *window*, the measured duty and the *displayed* level — a still is an interval, not a sample, so a reader compares a trace over `window_ns` rather than at `sim_ns`. The window is the one value the host cannot know (`DutyTracker.window`, sent in the child's `state` payload); the marker is emitted in both viewer dialects, with the tick count derived from the dump's **own** `$timescale` — read, never assumed, and **both backends gzip-wrap their FSTs**, so the header block is at offset 0 of the *decompressed* stream |
...
| `src/fpga_sim/stall.py` | "It looks frozen" (U48): `StallWatch` (quiet outputs **and** advancing simulated time — the second clause is what stops a dead child being blamed on the student's divider) + `stall_message` (every figure measured on this machine, never a constant) + `divider_bits` (a *name* heuristic over the design's generics; it only ever changes an advisory's wording, never a decision) + `faster_backend` (order only — `mcode` → `llvm-jit` → `llvm` → `nvc`; the *ratio* is machine- and design-dependent so the message never states one, and an unrecognized backend is ignored rather than assumed slow). pygame-free. **The simulator is selected in exactly one place — the preview's `SIM:` toggle.** The advisory points at it and deliberately has no [Switch to NVC] button of its own: switching engines mid-run tears the child down and restarts simulated time, so an in-panel button would read as "go faster" while discarding the wait the reader had already done |
| `src/fpga_sim/generics.py` | The generic override (U48/D-9): which generics a person may be offered (integers / boolean / single-bit; anything else listed read-only) and whether what they typed is worth handing to a simulator. Holds no UI and no simulator knowledge. The board-sized `NUM_*` generics are reserved; `COUNTER_BITS` is deliberately offered |
...
| `src/fpga_sim/sim_input.py` | Bounded input-edge queue shared by the headless child and its tests: buffers drained `input` messages so the child applies **at most one state per loop iteration** (each value therefore gets one `await Timer` of exposure), dropping consecutive duplicate states and hard-capping the backlog; pygame- and cocotb-free (U44, #353) |
...
| `sim/input_probe.vhd` | Test fixture that counts **rising edges** on `btn(0)` into `led` and never clears, so `tests/test_input_edges.py` can assert on a permanent record of what the DUT actually saw (a design that merely mirrors `btn` cannot tell a swallowed press from no press); parked under `sim/` so the picker never lists it (U44, #353) |
...
| `hdl/native/` | Board-native reference designs (a board's own port names + fixed widths, no `NUM_*`): Terasic `de10_standard.vhd`, `de0.vhd`, `de25_standard.vhd`; litex `arty_litex.vhd` (LiteX names `clk100`/`user_led`/`user_sw`/`user_btn`, U32); `arty_rgb.vhd` (Arty A7-100 native RGB channel scalars `led0_r`..`led3_b` via the `leds_rgb` bank, U38 — color wheel + `btn(0)` lamp test); scan-display pair `nexys4ddr_scan.vhd` (physical `CA..CG`/`DP`/`AN[7:0]` scalars) + `basys3_scan.vhd` (shared-vector `seg`/`dp`/`an` idiom) — U22 hex counter + `BTNC` lamp test, scanning fast (128-clock digit slots) per the scan-rate rule. Each matches via a board's `port_conventions` and is not in the file picker (U21) |
...
| `sim/test_native_scan.py` | Headless cocotb duty tests for the U22 native scan designs (1/N scan brightness, dp gating, lamp test), run against the Full-duty native wrapper by `tests/test_native_scan_design.py` under GHDL + NVC; digit count inferred from the boundary, so one module serves the 8-digit Nexys and 4-digit Basys |
...
| `docs/install.md` | Full install reference: GHDL/NVC per-OS matrix (incl. from-source, AUR/Gentoo/FreeBSD), uv setup, Windows run notes + PATH/DLL/MSYS2 troubleshooting, pygame-ce note (the GUI dependency, plus the pip-collision trap) |
...
3. When simulation starts, `ScreenController.on_simulate()` (the launcher window stays open) calls `start_simulation()` in `src/fpga_sim/sim_runner.py`, which returns a `SimChild` handle, then renders a `SimulationScreen` in the same window and calls `finish_waveform()` when it returns.

4. `sim_runner.py` builds a platform-aware environment (PATH, LD_LIBRARY_PATH/PYTHONHOME, VPI paths) and spawns a **headless** `ghdl -r ... --vpi=cocotbvpi_ghdl.so` child (no display). The board JSON rides in `FPGA_SIM_BOARD_JSON`; the IPC link port + authkey ride in `FPGA_SIM_LINK_PORT` / `FPGA_SIM_LINK_KEY`. Both `src/` and `sim/` are on `PYTHONPATH` so the child can import `fpga_sim` and find `sim_testbench`.
