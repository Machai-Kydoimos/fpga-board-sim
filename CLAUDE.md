...
| `src/fpga_sim/ui/inspect.py` | Inspect mode (U55): **F3** paints a typable address on every region (`sim.panel.speed.slider`), **F4** copies it plus a context stamp. Two orthogonal halves — an address that encoded state would be untypable, state left out would make it unreproducible. **Identity is never geometry** (`_layout` reassigns every rect on resize), and the registry is a module global like `THEME`/`_DEBUG_VIEW` so `draw_button` — one primitive for **17 call sites** — registers without being handed one. Three kinds: `zone` (frame + label), `item` (chrome, badged), `widget` (board parts, **never badged** — 57 names are less readable than none). Every entry point is a no-op on one bool read while off; colliding labels are dropped, never stacked. Colors are fixed, not THEME roles: a theme could restyle diagnostic chrome into invisibility |
