"""sim_testbench_bridge.py - headless cocotb bridge (no pygame, no window).

Single-window simulation (docs/experiments/single_window_sim.md): the pygame UI
stays in the launcher process, which owns the one and only window.  This module
runs inside the GHDL/NVC process *instead of* ``sim_testbench`` and only
shuttles signal state over the ``sim_link`` connection; it supersedes the
in-child pygame UI.  It never imports pygame, directly or transitively -- its
only ``fpga_sim`` imports are ``board_loader``, ``sim_link`` and (when metrics
are enabled) ``sim_metrics``, all pygame-free.  Each frame it:

  1. computes the sim step from the current speed factor
     (same ``_REAL_STEP_NS`` / ``_MAX_CYCLES_PER_STEP`` math as sim_testbench)
  2. ``await Timer(step)``
